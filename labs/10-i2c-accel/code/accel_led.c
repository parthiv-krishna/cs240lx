...
void notmain(void) {
    ...
    while (1) {
        ...
        imu_xyz_t xyz = accel_scale(&h, xyz_raw);

        unsigned x = xyz.x > 0 ? xyz.x : -xyz.x;
        unsigned y = xyz.y > 0 ? xyz.y : -xyz.y;
        unsigned z = xyz.z > 0 ? xyz.z : -xyz.z;
        for (int i = 3; i < 16; i++) {
            neopix_write(neo, i, x / 256, y / 256, z / 256);
        }
        neopix_flush(neo);
